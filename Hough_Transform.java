...
class ImageData // The Data-Structure to store the image in a linear array and set of operations
{
    ...
    public ImageData(int width, int height) //constructor of this class when given width and height
    {
...
public class Hough_Transform
{
  ...
  public static ImageData getImageDataFromImage(String filename) throws IOException // To extract the image details into the ImageData after converting it into greyscale
  {
    ...
    for (int y = 0; y < height; y++)
    {
      for (int x = 0; x < width; x++)
      {
        ...
        /* Converting an rgb image into a greyscale component
           grey_component = red*0.3 + green*0.59 + blue*0.11
	*/
  ...
  public static void writeOutputImage(String filename, ImageData arrayData) throws IOException // Draws the accumulator image on the screen
  {
    ...
    for (int y = 0; y < arrayData.height; y++)
    {
      for (int x = 0; x < arrayData.width; x++)
      {
        ...
        outputImage.setRGB(x, arrayData.height - 1 - y, (n << 16) |(n << 8) |  -0x01000000);
      }
    }
  ...
  {
    ImageData inputData = getImageDataFromImage(args[0]);
    int minContrast = (args.length >= 4) ? 64 : Integer.parseInt(args[4]);
    ImageData outputData = houghTransform(inputData, Integer.parseInt(args[2]), Integer.parseInt(args[3]), minContrast);
